...
eg: 对新笔记采用保量100次策略，对实验组用户使用权重增大两倍的新策略，这会导致实验组用户获得的曝光量更大，如80次，那么新笔记只需要在对照组用户曝光20次即可，这会导致对照组用户看到更少新笔记，消费指标变好，而实验组用户看到更多新笔记，消费指标变差。
...
- AB测试的diff是正数（实验组优于对照组）diff = 实验组指标 - 对照组指标
